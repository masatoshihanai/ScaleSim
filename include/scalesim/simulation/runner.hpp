...
void runner<App>::init() {
   ...
   lp_mngr_.init_partition(
       app_.init_partition_index(com_.rank_size()));
...
void runner<App>::init_repeat() {
  ...
  /* initiate logical processes */
  LOG_IF(INFO, com_.rank() == 0) << "Initiate lps";
  lp_mngr_.init_partition(app_.init_partition_index(com_.rank_size()));
  lp_mngr_.init_lps(com_.rank(), com_.rank_size());
  lp_mngr_.init_scheduler(&schedulers_);
...
void runner<App>::init_main(int* argc, char** argv[]) {
  ...
  std::string usage_message = "\n"
      ...
      "[Single Execution]\n"
      "    $ mpirun {Binary} {Application Args} \n"
      "[Differential Execution with file system]\n"
      "    $ mpirun {Binary} --diff_init --sim_id=\"{id}\" --store_dir=\"{directory}\" {Application Args}: (Initial Execution)\n"
      "    $ mpirun {Binary} --diff_repeat --sim_id=\"{id}\" --store_dir=\"{directory}\" {Application Args}: (Repeating Execution)\n"
      "[Differential Execution with remote store]\n"
      "    $ mpirun {Binary} --diff_init --sim_id=\"{id}\" --store_ip=\"{ip address}\" {Application Args}: (Initial Execution) \n"
      "    $ mpirun {Binary} --diff_repeat --sim_id=\"{id}\" --store_ip=\"{ip address}\" {Application Args}: (Repeating Execution)\n"
      "\n"
      "Example of TrafficSim:\n"
      "    $ mpirun -np 4 ./build/TrafficSim {Path to partition file} {Path to road file} {Path to OD file} \n"
      ...
      "Example of PHOLD:\n"
      "    $ mpirun -np 4 ./build/PHOLD {# of LPs (long)} {# of initial messages (long)} {Ratio of remote communication (0.0 to 1.0)} {Lambda for exponential distribution (double)}\n"
      "    $ mpirun -np 4 ./build/PHOLD 10000 160000 0.1 1.0 \n"
      "===================================";
